...
### Server Build Step
#####################
FROM --platform=${TARGETPLATFORM:-linux/amd64} golang:1.18-bullseye AS server-builder 
...
### UI Build Step
#################
FROM --platform=${TARGETPLATFORM:-linux/amd64} node:16-bullseye AS ui-builder 
